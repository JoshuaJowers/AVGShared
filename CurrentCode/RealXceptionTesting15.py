...
#How do I have 26000 images and Wint only had 10,000?
import torch
...
def negative_log_likelihood_loss(predictions, targets):
    ...
    min_std = 0.3989422
    # min_prediction = 0.0
    std_x = torch.clamp(std_x, min_std)
    std_y = torch.clamp(std_y, min_std)
    # mean_x = torch.clamp(mean_x, min_prediction, 1000)
    # mean_y = torch.clamp(mean_y, min_prediction, 1000)
    # Calculate the PDFs for x and y
    pdf_x = (1 / (std_x * torch.sqrt(2 * torch.tensor(math.pi)))) * torch.exp(-0.5 * ((target_x - mean_x) / (std_x)) ** 2)
    pdf_y = (1 / (std_y * torch.sqrt(2 * torch.tensor(math.pi)))) * torch.exp(-0.5 * ((target_y - mean_y) / (std_y)) ** 2)

    # Calculate the negative log likelihood loss
    loss_x = -torch.log(pdf_x)
    loss_y = -torch.log(pdf_y)
...
valid_length = dataset_size - train_length
...
for epoch in range(epochs):
   ...
   for batch_idx, (images, targets) in enumerate(train_loader):
       ...
       loss = negative_log_likelihood_loss(output,targets)
   ...
   with torch.no_grad():
       ...
       if epoch % 10 < 10:
           for images, targets in valid_loader:
               ...
               loss2 = negative_log_likelihood_loss(output, targets)
       ...
       loss_in_meters = loss * 32200
       with open(output_file_name, 'a') as output_file:
            output_file.write('Validation SL Loss: %.3f' % (val_loss / len(valid_loader)) + "\n")
            output_file.write('Loss in Meters: %3f' % (loss_in_meters / len(valid_loader)) + "\n")
            output_file.write('Validation NLL Loss: %.3f' % (val_loss2 / len(valid_loader)) + "\n")
